...
class EducationDB:
    def __init__(self, max_retries=10, retry_delay=5):
        ...
        self.db_config = {
            "host": os.getenv("DATABASE_HOST", "postgres"),  # ← ИЗМЕНИТЕ на "postgres"
    ...
    def ensure_database_exists(self):
        ...
        try:
            ...
            # ИСПОЛЬЗУЕМ ТЕ ЖЕ ПАРАМЕТРЫ, ЧТО И В ОСНОВНОМ ПОДКЛЮЧЕНИИ
            temp_conn = psycopg2.connect(
                host=self.db_config["host"],  # ← ИСПОЛЬЗУЕМ self.db_config
            ...
            # НЕ завершаем программу, возможно БД уже создана
            # sys.exit(1)
